...
>>>>>>> aad53586961d41ee1e248d31d0deaabcc8ad09ca
  ...
  def test_wcdb_login2 (self):
    a1 = ("z","joshen","pb6bKYnCDt","cs327e_joshen")
    login = wcdb_login(*a1)
    self.assertTrue(str(type(login)) != "<type '_mysql.connection'>")
        
  def test_wcdb_login3 (self):
    a2 = ("z","joshen1","pb6bKYnCDs","cs327e_joshen")
    login = wcdb_login(*a2)
    self.assertTrue(str(type(login)) != "<type '_mysql.connection'>")
    
  def test_wcdb_login4 (self):
    a3 = ("z","joshen","pb6bKYnCDs","cs327e_joshen1")
    login = wcdb_login(*a3)
    self.assertTrue(str(type(login)) != "<type '_mysql.connection'>")
  ...
  def test_wcdb_import2(self):
    login_var = wcdb_login(*a)
    createDB(login_var)
    tree = ET.parse(StringIO("""<root>
    <crises>
        <crisis>
            <crisisId>CRI_030</crisisId>
  ...
  def test_wcdb_import3(self):
    ...
    tree = ET.parse(StringIO("""
                                <root>
                                  <citations>
                                    <citationPair>
                                      <citationId>CIT_001</citationId>
                                      <citation>http://www.ccoc.net/the-chernobyl-disaster/the-story</citation>
                                    </citationPair>
                                  </citations>
                                </root>
                              """))
    wcdb_import(login_var, tree)
    self.assertTrue(len(wcdb_query(login_var, "select * from Citations;")) == 1)
  ...
  def test_wcdb_read_tag(self):
    ...
    root = wcdb_read(r, flag = True, filename = " ")
  ...
  def test_wcdb_write_single_ele(self):
      w = StringIO()
      aa = ET.Element('WorldCrises')
      b = ET.SubElement(aa,'apple')
      wcdb_write(w, aa)
  ...
  # ----------------------------------------------------
  # wcdb_export function test 3 in total
  # ----------------------------------------------------
...
##                         'TeamRocket-WCDB.xml', \  # their data does not have kind tag
  ...
  def test_wcdb_solve_1(self):
      ...
      wcdb_solve(r, w1, xml_filename_list)
      r2 = StringIO(w1.getvalue())
      wcdb_solve(r2, w2, xml_filename_list)
